...
export default function Home() {
  ...
  const [screen, setScreen] = useState<"game" | "diagnosis" | "recruit" | "complete">("game");
  ...
  const [runHistory, setRunHistory] = useState<Array<{ game: number; won: boolean; diagnosis: string[]; recruit?: string }>>([]);
  ...
  function recordRun() {
    setRunHistory((history) => [...history, { game: gameNumber, won: Boolean(game.outcome?.won), diagnosis: diagnosis(game) }]);
  ...
  function confirmRecruit() {
    ...
    setRunHistory((history) => history.map((run, index) => index === history.length - 1 ? { ...run, recruit: chosenRecruit } : run));
  }
  function beginNextGame() {
    ...
    setGame(startGame(createGame({ ...conditions, sequence: nextSequence, lineup, seed: conditions.seed + nextNumber - 1 })));
  }
  function replayGame() { setScreen("game"); setGame(startGame(createGame(game.conditions))); }

  return <main className="site-shell">
    <header className="masthead"><div><p className="eyebrow">SINBIN // MECHANIC TEST 03</p><h1>THREE-GAME SHIFT</h1></div><div className="lab-thesis"><span>THE TEST</span><strong>Does a diagnosed failure make you recruit the missing component?</strong></div></header>
    ...
    <div className="game-layout"><section className="play-column">
      ...
      <div className="action-deck"><button className="secondary-action" disabled={game.phase !== "running"} onClick={() => setGame(dumpAndChange)}><span>DUMP &amp; FULL CHANGE</span><small>Surrender Pressure · reset Chain · clear Threat</small></button><button className="danger-action" disabled={game.phase !== "running" || !selectedPlayer || Boolean(game.sinBin) || Boolean(selected && game.penaltiesUsed.includes(selected))} onClick={() => setGame(takePenalty)}><span>{selectedPlayer ? `SEND ${selectedPlayer.name} TO THE SIN BIN` : "SELECT SKATER // INTENTIONAL PENALTY"}</span><small>Overcharge · two exposed resolutions · return</small></button><button className="pause-action" disabled={game.phase !== "running"} onClick={() => setGame(resolve)}>RESOLVE NOW</button></div>
    ...
    {screen === "game" && game.phase === "ended" && <div className="result-backdrop"><section className={`result-card ${game.outcome?.won ? "win" : "loss"}`}><p className="eyebrow">GAME {gameNumber} DIAGNOSIS · {SEQUENCES[game.conditions.sequence as SequenceId].name}</p><h2>{game.outcome?.won ? "THE MACHINE HELD" : "THE PLAY CAME APART"}</h2><div className="result-score"><strong>{game.goalsFor}</strong><span>—</span><strong>{game.goalsAgainst}</strong></div><div className="diagnosis-list">{diagnosis(game).map((line: string) => <p key={line}>{line}</p>)}</div><button className="primary-action" onClick={recordRun}>{gameNumber === 3 ? "COMPLETE THREE-GAME TEST" : "FACE THE RECRUITS"}</button><button className="text-action" onClick={replayGame}>Replay identical game</button></section></div>}

    {screen === "recruit" && <div className="result-backdrop"><section className="result-card recruit-screen"><p className="eyebrow">BETWEEN GAMES · DIAGNOSE → RECRUIT → RELEASE → ORDER</p><h2>WHAT WAS MISSING?</h2><p className="result-subtitle">Choose a future for the machine. These are mechanical alternatives, not a power ladder.</p><div className="recruit-grid">{RECRUIT_OFFERS[gameNumber - 1].map((id) => <SkaterCard key={id} player={players[id]} selected={chosenRecruit === id} onClick={recruitConfirmed ? undefined : () => setChosenRecruit(id)} label={id === RECRUIT_OFFERS[gameNumber - 1][0] ? "POSSESSION BRIDGE" : id === RECRUIT_OFFERS[gameNumber - 1][1] ? "STABILIZER" : "VOLATILE SPECIALIST"} />)}</div><span className="label intermission-label">RELEASE ONE CURRENT SKATER</span><div className="release-grid">{[...lineup.active, ...lineup.bench].map((id) => <button disabled={recruitConfirmed} className={releaseId === id ? "chosen" : ""} key={id} onClick={() => setReleaseId(id)}><strong>{players[id].name}</strong><small>{players[id].role}</small></button>)}</div>{!recruitConfirmed ? <button className="primary-action" disabled={!chosenRecruit || !releaseId} onClick={confirmRecruit}>CONFIRM RECRUITMENT</button> : <><span className="label intermission-label">SET BENCH ORDER · EARLIER CARDS ARE EASIER TO FIND LIVE</span><div className="bench-order">{lineup.bench.map((id, index) => <div key={id}><b>{index + 1}</b><span><strong>{players[id].name}</strong><small>{players[id].role}</small></span><button disabled={index === 0} onClick={() => setLineup((current) => moveBenchSkater(current, index, -1))}>↑</button><button disabled={index === lineup.bench.length - 1} onClick={() => setLineup((current) => moveBenchSkater(current, index, 1))}>↓</button></div>)}</div><button className="primary-action" onClick={beginNextGame}>START GAME {gameNumber + 1} · {SEQUENCES[campaignSequences[gameNumber]].name}</button></>}</section></div>}

    {screen === "complete" && <div className="result-backdrop"><section className="result-card"><p className="eyebrow">LSL-3 · THREE-GAME DIAGNOSIS</p><h2>RUN COMPLETE</h2><div className="campaign-summary">{runHistory.map((run) => <div key={run.game}><strong>GAME {run.game} · {run.won ? "HELD" : "BROKE"}</strong><p>{run.diagnosis.at(-1)}</p><small>{run.recruit ? `Recruited ${players[run.recruit].name} for the next game.` : "Final game."}</small></div>)}</div><button className="primary-action" onClick={() => resetCampaign()}>RUN A NEW THREE-GAME TEST</button></section></div>}
    <footer><span>SINBIN // LIVE SHIFT LAB 3</span><span>NO CURRENCY. NO UPGRADES. ONE MISSING COMPONENT.</span></footer>
  </main>;
}
