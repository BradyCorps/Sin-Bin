...
export function createGame(conditions = {}) {
  ...
  return {
    ...
    selectedSlot: null, sinBin: null, penaltiesUsed: [], changedThisResolution: false, windowDecision: null, lastBridge: false, goalsFor: 0, goalsAgainst: 0,
...
export function dumpAndChange(state) {
  if (state.phase !== "running" || state.windowDecision) return state; const next = copy(state); const surrendered = Math.min(18, Math.round(next.pressure));
  ...
  next.pressure = Math.max(0, next.pressure - surrendered); next.threat = Math.max(0, next.threat - 4); next.chain = 1; next.selectedSlot = null; next.changedThisResolution = true; next.windowDecision = "dump"; next.lastBridge = false;
...
export function takePenalty(state) {
  if (state.phase !== "running" || state.windowDecision || state.selectedSlot === null || state.sinBin) return state; const id = state.active[state.selectedSlot];
  ...
  next.active[slot] = null; next.sinBin = { id, slot, remaining: PENALTY_RESOLUTIONS, conceded: false }; next.penaltiesUsed.push(id); next.selectedSlot = null; next.windowDecision = "penalty"; next.pressure += overcharge; next.chain = clamp(next.chain + 0.25, 1, 4.5); next.stats.penalties += 1;
...
export function resolve(state) {
  ...
  if (next.windowDecision === "dump") { scoreThreat(next, 12, "The dumped puck let the opponent regroup."); next.chain = 1; record(next, "dump-resolution", "Full Change surrendered this scoring attempt; opponent regroup +12 Threat."); }
  ...
  next.changedThisResolution = false; next.windowDecision = null; next.lastBridge = false; next.selectedSlot = null; next.maxChain = Math.max(next.maxChain, next.chain); next.clockMs = CADENCES[next.conditions.cadence];
